...
//!
//! Unlike standard LEA-128 (which uses 24+ rounds), this version uses **12 rounds** for speed,
...
pub fn lea_encrypt_str(input: &str, key: &[u8; 16]) -> Vec<u8> {
    ...
    cipher.encrypt_blocks(out.as_mut_ptr(), out.len());
    out
}

/// Decrypts a byte buffer using the given key and returns the original UTF-8 string.
pub fn lea_decrypt_bytes(encrypted: &[u8], key: &[u8; 16]) -> String {
    let cipher = LEA128::new(key);

    let mut buf = encrypted.to_vec();
    cipher.decrypt_blocks(buf.as_mut_ptr(), buf.len());
